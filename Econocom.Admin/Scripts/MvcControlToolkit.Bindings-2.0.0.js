...
(function () {
    ...
    function propertySet(item, name, value) {
        name = name.split(".");
        var res = item;
        for (var i = 0; i < name.length - 1; i++) {
        ...
        if (ko.isObservable(res[name[name.length - 1]]))
...
function MvcControlsToolkit_NewTemplateName(item, data) {
    if (eval("typeof _MvcControlsToolkit_" + item.ModelId + " === 'undefined'")) {
        jQuery.globalEval("var _MvcControlsToolkit_" + item.ModelId + " = 0;");
        data._tag = data["_tag"] || eval("_MvcControlsToolkit_" + item.ModelId);
    }
    else {
        var tag = data["_tag"] || null;
        if (!tag) {
            eval("_MvcControlsToolkit_" + item.ModelId + "++ ;");
            data._tag = eval("_MvcControlsToolkit_" + item.ModelId)
        }
    }
...
function MvcControlsToolkit_TemplateName(item, data) {
    return item["Single"] || false ?
        item.ModelPrefix :
        item.ModelPrefix + "[" + data["_tag"] + "]" + item.ItemPrefix;
}
function MvcControlsToolkit_TemplateId(item, data) {
    return item["Single"] || false ?
        item.ModelId :
        item.ModelId + "_" + data["_tag"] + "_" + item.ItemPrefix; ;
}
///////////////////////////Templates/////////////////////////////
